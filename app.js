...
window.switchSimTab = function(tabKey) {
    ...
    tabs.forEach(t => {
        ...
        const btn = document.getElementById(`simTabBtn-${t}`) || document.getElementById(`tab-${t}`);
        if (pane) {
            if (t === tabKey) {
...
window.switchTab = window.switchSimTab;

window.setSimAmount = function(amount) {
    const formatted = `₹${Number(amount).toLocaleString('en-IN')}.00`;
    const storeVal = document.getElementById('simStoreVal');
    const gatewayFee = document.getElementById('simGatewayFee');
    const bankVal = document.getElementById('simBankVal');
    
    if (storeVal) storeVal.innerText = formatted;
    const feeVal = (amount * 0.0236).toFixed(2);
    const netVal = (amount - feeVal).toFixed(2);
    
    if (gatewayFee) gatewayFee.innerText = `- ₹${Number(feeVal).toLocaleString('en-IN')}`;
    if (bankVal) bankVal.innerText = `₹${Number(netVal).toLocaleString('en-IN')}`;
    
    if (typeof showToast === 'function') {
        showToast(`⚡ Simulator: Set Order Amount to ${formatted}`);
    }
};
